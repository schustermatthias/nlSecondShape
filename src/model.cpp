...
void kernel_constant_neumann(const double *x, const long labelx, const double *y, const long labely,
                             const MeshType &mesh, double *kernel_val) {
    if (labelx == 1 or labely == 1) {
        *kernel_val = 4 / (M_PI * pow(mesh.sqdelta, 2));
    } else {
        *kernel_val = 0.0;
    }
...
void kernel_schwarz_neumann(const double *x, const long labelx, const double *y, const long labely,
                            const MeshType &mesh, double *kernel_val) {
    if (labelx == 3 and labely == 3) {
        *kernel_val = 0.0;
    } else {
        *kernel_val = 4 / (M_PI * pow(mesh.sqdelta, 2));
    }
...
void kernel_constantTruncated(const double *x, const long labelx, const double *y, const long labely, const MeshType &mesh,
    ...
    *kernel_val = doesInteract * 2 / (M_PI * pow(mesh.sqdelta, 2));
}
...
void kernel_integrable_sym(const double * x, const long labelx, const double * y, const long labely,
    ...
    dist = vec_sqL2dist(x, y, 2);
    if (label == 11) {
        *kernel_val = 0.1 * 1. / (pow(mesh.sqdelta, 2));
    } else if (label==22){
        *kernel_val = (10 *  1. / (pow(mesh.sqdelta, 2)));// * (1 - sqrt((dist/mesh.sqdelta)) );
    } else {
...
void kernel_integrable_unsym_1(const double * x, const long labelx, const double * y, const long labely,
                               const MeshType &mesh, double * kernel_val){
    *kernel_val = 3.0 * 1. / (pow(mesh.sqdelta, 2));
}


void kernel_integrable_unsym_2(const double * x, const long labelx, const double * y, const long labely,
                               const MeshType &mesh, double * kernel_val){
...
void kernel_singular(const double * x, const long labelx, const double * y, const long labely,
                     const MeshType &mesh, double * kernel_val) {
    const double exponent = mesh.dim+2.*mesh.fractional_s;
    const double constant_exponent = 2. - 2.*mesh.fractional_s;
    double z[mesh.dim];
    ...
    const double denominator = 1.0/pow(sqrt(vec_dot(z,z,mesh.dim)), exponent);
    // Constant is wrong I guess..
    const double c = 2.0 * (constant_exponent)/(M_PI*pow(mesh.delta, constant_exponent));
    *kernel_val = c*denominator;
}


void kernel_DEliaGlusa(const double * x, const long labelx, const double * y, const long labely,
                       const MeshType &mesh, double * kernel_val) {
    const double exponent = mesh.dim+2.*mesh.fractional_s;
