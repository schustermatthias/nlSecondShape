...
int ERROR_wrongAccess_shape(const ElementType &aT, const ElementType &bT, const QuadratureType &quadRule, const MeshType &mesh,
                      ...
                      const int CaseShapeDerivative){
    cout << "ERROR in integration.cpp/ERROR_wrongAccess_shape(): You chose no integration method for computing the shape derivative, but the assembly routine tried to call it." << endl;
    cout << "The names of possible choices are given in lookup_configuration() in Cassemble.cpp." << endl;
    abort();
    return -1;
};
...
int integrate_shape(const ElementType &aT, const ElementType &bT,
              ...
              bool is_firstbfslayer, double *termLocal,
              double *termNonloc, double *termLocalPrime, double *termNonlocPrime,
              double * state_nodal_values, double * adjoint_nodal_values, const int CaseShapeDerivative){
    if (is_firstbfslayer) {
...
int integrate_fractional_shape(const ElementType &aT, const ElementType &bT, const QuadratureType &quadRule,
    ...
    int num_vertices = 2*mesh.dVertex;
    double c = mesh.outdim + 2*mesh.fractional_s;
    ...
    for(int traffoCounter=0; traffoCounter < nTraffos; traffoCounter++) {
        ...
        for (int k = 0; k < quadRule.nPg; k++) {
            ...
            for (int a = 0; a < nEqual; a++){
                termNonloc[argSortA[a]] += -c * factors * psixy[a]
                        * (x_canceled[0] - y_canceled[0])/(vec_sqL2dist(x_canceled, y_canceled, 2));
                termNonloc[argSortA[a] + num_vertices] += -c * factors * psixy[a]
            ...
            for (int a = nEqual; a < mesh.dVertex; ++a) {
                ...
                termNonloc[argSortA[a] + num_vertices] += -c * factors * psixy[a]
                                                          * (x_canceled[1] - y_canceled[1])/(vec_sqL2dist(x_canceled, y_canceled, 2));
                termNonloc[argSortB[a] + mesh.dVertex] += -c * factors * psixy[a + shift]
                                           * (x_canceled[0] - y_canceled[0])/(vec_sqL2dist(x_canceled, y_canceled, 2));
                termNonloc[argSortB[a] + mesh.dVertex + num_vertices] += -c * factors * psixy[a + shift]
                                                          * (x_canceled[1] - y_canceled[1])/(vec_sqL2dist(x_canceled, y_canceled, 2));
            }
        }
    }
...
int integrate_fractional_second_shape(const ElementType &aT, const ElementType &bT, const QuadratureType &quadRule,
                                      const MeshType &mesh, const ConfigurationType &conf, bool is_firstbfslayer,
                                      double * termLocal, double * termNonloc, double *termLocalPrime, double *termNonlocPrime,
                                      double * state_nodal_values, double * adjoint_nodal_values, const int CaseShapeDerivative){
    //printf("Ok, is_firstbfslayer!");
        ...
        for (int k = 0; k < quadRule.nPg; k++) {
            ...
            if (CaseShapeDerivative == 1) {
                ...
                if (conf.ShapeDerivative == 1) {
                    ...
                    value11 = factors * (-c * 1 / (vec_sqL2dist(x_canceled, y_canceled, 2)) +
                                                       (mesh.outdim * c + pow(c, 2))
                                                       * (pow(x_canceled[0] - y_canceled[0], 2)) /
                                                       pow(vec_sqL2dist(x_canceled, y_canceled, 2),2));
                    value22 = factors * (-c * 1 / (vec_sqL2dist(x_canceled, y_canceled, 2)) +
                                                       (mesh.outdim * c + pow(c, 2))
                                                       * (pow(x_canceled[1] - y_canceled[1], 2)) /
                                                       pow(vec_sqL2dist(x_canceled, y_canceled, 2),2));
                    value12 = factors * (-c * 1 / (vec_sqL2dist(x_canceled, y_canceled, 2)) +
                                                       (mesh.outdim * c + pow(c, 2))
                                                       * ((x_canceled[0] - y_canceled[0]) *
                                                          (x_canceled[1] - y_canceled[1])) /
                                                       pow(vec_sqL2dist(x_canceled, y_canceled, 2),2));

                    for (int a = 0; a < nEqual; a++) {
                        for (int b = 0; b < nEqual; b++) {
                            // V^T Hess W
                            //both basis functions with supp on both triangle
                            termLocalPrime[argSortA[a] * 2 * num_vertices + argSortA[b]] += value11 * psixy[a] * psixy[b];
                            termLocalPrime[argSortA[a] * 2 * num_vertices + argSortA[b] + num_vertices] +=
                                    value12 * psixy[a] * psixy[b];
                            termLocalPrime[(argSortA[a] + num_vertices) * 2 * num_vertices + argSortA[b]] +=
                                    value12 * psixy[a] * psixy[b];
                            termLocalPrime[(argSortA[a] + num_vertices) * 2 * num_vertices + argSortA[b] + num_vertices] +=
                                    value22 * psixy[a] * psixy[b];

                        }
                        for (int b = nEqual; b < mesh.dVertex; b++) {
                            // V^T Hess W
                            //first function supp on both triangle, second function supp on first triangle
                            termLocalPrime[argSortA[a] * 2 * num_vertices + argSortA[b]] += value11 * psixy[a] * psixy[b];
                            termLocalPrime[argSortA[a] * 2 * num_vertices + argSortA[b] + num_vertices] +=
                                    value12 * psixy[a] * psixy[b];
                            termLocalPrime[(argSortA[a] + num_vertices) * 2 * num_vertices + argSortA[b]] +=
                            ...
                            //first function supp on both triangle, second function supp on second triangle
                            termLocalPrime[argSortA[a] * 2 * num_vertices + argSortB[b] + mesh.dVertex] +=
                                    value11 * psixy[a] * psixy[b + shift];
                            termLocalPrime[argSortA[a] * 2 * num_vertices + argSortB[b] + num_vertices + mesh.dVertex] +=
                                    value12 * psixy[a] * psixy[b + shift];
                            termLocalPrime[(argSortA[a] + num_vertices) * 2 * num_vertices + argSortB[b] + mesh.dVertex] +=
                                    value12 * psixy[a] * psixy[b + shift];
                            termLocalPrime[(argSortA[a] + num_vertices) * 2 * num_vertices + argSortB[b] + num_vertices +
                                           mesh.dVertex] += value22 * psixy[a] * psixy[b + shift];
                        }

                        // grad_x kernel^T(V-V') and grad_x kernel^T(W-W')
                        termNonlocPrime[argSortA[a]] += -c * factors * psixy[a]
                                                        * (x_canceled[0] - y_canceled[0]) /
                                                        (vec_sqL2dist(x_canceled, y_canceled, 2));
                        termNonlocPrime[argSortA[a] + num_vertices] +=
                                -c * factors * psixy[a]
                                * (x_canceled[1] - y_canceled[1]) / (vec_sqL2dist(x_canceled, y_canceled, 2));
                    }
                    ...
                    for (int a = nEqual; a < mesh.dVertex; a++) {
                        for (int b = 0; b < nEqual; b++) {
                            // V^T Hess W
                            //first function supp on both triangle, second function supp on both triangle
                            termLocalPrime[argSortA[a] * 2 * num_vertices + argSortA[b]] += value11 * psixy[a] * psixy[b];
                            termLocalPrime[argSortA[a] * 2 * num_vertices + argSortA[b] + num_vertices] +=
                                    value12 * psixy[a] * psixy[b];
                            termLocalPrime[(argSortA[a] + num_vertices) * 2 * num_vertices + argSortA[b]] +=
                            ...
                            //first function supp on second triangle, second function supp on both triangle
                            termLocalPrime[(argSortB[a] + mesh.dVertex) * 2 * num_vertices + argSortA[b]] +=
                                    value11 * psixy[a + shift] * psixy[b];
                            termLocalPrime[(argSortB[a] + mesh.dVertex) * 2 * num_vertices + argSortA[b] + num_vertices] +=
                                    value12 * psixy[a + shift] * psixy[b];
                            termLocalPrime[(argSortB[a] + mesh.dVertex + num_vertices) * 2 * num_vertices + argSortA[b]] +=
                                    value12 * psixy[a + shift] * psixy[b];
                            termLocalPrime[(argSortB[a] + mesh.dVertex + num_vertices) * 2 * num_vertices + argSortA[b] +
                                           num_vertices] += value22 * psixy[a + shift] * psixy[b];
                        }
                        for (int b = nEqual; b < mesh.dVertex; b++) {
                            // V^T Hess W
                            //both basis functions with supp on first triangle
                            termLocalPrime[argSortA[a] * 2 * num_vertices + argSortA[b]] += value11 * psixy[a] * psixy[b];
                            termLocalPrime[argSortA[a] * 2 * num_vertices + argSortA[b] + num_vertices] +=
                                    value12 * psixy[a] * psixy[b];
                            termLocalPrime[(argSortA[a] + num_vertices) * 2 * num_vertices + argSortA[b]] +=
                            ...
                            //first function supp on first triangle, second function supp on second triangle
                            termLocalPrime[argSortA[a] * 2 * num_vertices + argSortB[b] + mesh.dVertex] +=
                                    value11 * psixy[a] * psixy[b + shift];
                            termLocalPrime[argSortA[a] * 2 * num_vertices + argSortB[b] + num_vertices + mesh.dVertex] +=
                                    value12 * psixy[a] * psixy[b + shift];
                            ...
                            //both basis functions with supp on second triangle
                            termLocalPrime[(argSortB[a] + mesh.dVertex) * 2 * num_vertices + argSortB[b] + mesh.dVertex] +=
                                    value11 * psixy[a + shift] * psixy[b + shift];
                            termLocalPrime[(argSortB[a] + mesh.dVertex) * 2 * num_vertices + argSortB[b] + mesh.dVertex +
                                           num_vertices] += value12 * psixy[a + shift] * psixy[b + shift];
                            termLocalPrime[(argSortB[a] + mesh.dVertex + num_vertices) * 2 * num_vertices + argSortB[b] +
                                           mesh.dVertex] += value12 * psixy[a + shift] * psixy[b + shift];
                            termLocalPrime[(argSortB[a] + mesh.dVertex + num_vertices) * 2 * num_vertices + argSortB[b] +
                                           mesh.dVertex + num_vertices] += value22 * psixy[a + shift] * psixy[b + shift];

                            //first function supp on second triangle, second function supp on first triangle
                            termLocalPrime[(argSortB[a] + mesh.dVertex) * 2 * num_vertices + argSortA[b]] +=
                                    value11 * psixy[a + shift] * psixy[b];
                            termLocalPrime[(argSortB[a] + mesh.dVertex) * 2 * num_vertices + argSortA[b] + num_vertices] +=
                                    value12 * psixy[a + shift] * psixy[b];
                        ...
                        termNonlocPrime[argSortA[a]] += -c * factors * psixy[a]
                                              * (x_canceled[0] - y_canceled[0]) /
                                              (vec_sqL2dist(x_canceled, y_canceled, 2));
                        termNonlocPrime[argSortB[a] + mesh.dVertex] += -c * factors * psixy[a + shift]
                                                             * (x_canceled[0] - y_canceled[0]) /
                                                             (vec_sqL2dist(x_canceled, y_canceled, 2));
                        termNonlocPrime[argSortA[a] + num_vertices] +=
                                -c * factors * psixy[a]
                                * (x_canceled[1] - y_canceled[1]) / (vec_sqL2dist(x_canceled, y_canceled, 2));
                        termNonlocPrime[argSortB[a] + num_vertices + mesh.dVertex] +=
                                -c * factors * psixy[a + shift]
                                * (x_canceled[1] - y_canceled[1]) / (vec_sqL2dist(x_canceled, y_canceled, 2));
                    }
                }
            } else {
                for (int a = 0; a < nEqual; a++) {
                    if (CaseShapeDerivative == 2) {
                        value = kernel_val[0] * traffoFractionalCanceled * (state_values[0] - state_values[1])
                                * psixy[a] * quadRule.dg[k] * aTsorted.absDet * bTsorted.absDet;
                    } else {
                        value = kernel_val[0] * traffoFractionalCanceled * (adjoint_values[0] - adjoint_values[1])
                                * psixy[a] * quadRule.dg[k] * aTsorted.absDet * bTsorted.absDet;
                    }
                    termLocal[argSortA[a]] += value;
                    if (conf.is_singularKernel){
                        for (int b = 0; b < nEqual; ++b) {
                            termNonloc[argSortA[a]*2*num_vertices + argSortA[b]]+= -c * value
                                                                                   * (x_canceled[0] - y_canceled[0])/(vec_sqL2dist(x_canceled, y_canceled, 2))
                                                                                   * psixy[b];
                            termNonloc[argSortA[a]*2*num_vertices + argSortA[b] + num_vertices]+= -c * value
                                                                                                  * (x_canceled[1] - y_canceled[1])/(vec_sqL2dist(x_canceled, y_canceled, 2))
                                                                                                  * psixy[b];
                        }
                        for (int b = nEqual; b < mesh.dVertex; ++b) {
                            ...
                            termNonloc[argSortA[a]*2*num_vertices + argSortB[b] + mesh.dVertex]+= -c * value
                                                                                                  * (x_canceled[0] - y_canceled[0])/(vec_sqL2dist(x_canceled, y_canceled, 2))
                                                                                                  * psixy[b + shift];
                            termNonloc[argSortA[a]*2*num_vertices + argSortB[b] + mesh.dVertex + num_vertices]+= -c * value
                                                                                                                 * (x_canceled[1] - y_canceled[1])/(vec_sqL2dist(x_canceled, y_canceled, 2))
                                                                                                                 * psixy[b + shift];
                        }
                    }
                ...
                for (int a = nEqual; a < mesh.dVertex; a++) {
                    if (CaseShapeDerivative == 2) {
                        value = kernel_val[0] * traffoFractionalCanceled * (state_values[0] - state_values[1])
                                * psixy[a] * quadRule.dg[k] * aTsorted.absDet * bTsorted.absDet;
                        valuePrime = kernel_val[0] * traffoFractionalCanceled * (state_values[0] - state_values[1])
                                     * psixy[a + shift] * quadRule.dg[k] * aTsorted.absDet * bTsorted.absDet;
                    } else {
                        value = kernel_val[0] * traffoFractionalCanceled * (adjoint_values[0] - adjoint_values[1])
                                * psixy[a] * quadRule.dg[k] * aTsorted.absDet * bTsorted.absDet;
                        valuePrime = kernel_val[0] * traffoFractionalCanceled * (adjoint_values[0] - adjoint_values[1])
                                     *psixy[a + shift] * quadRule.dg[k] * aTsorted.absDet * bTsorted.absDet;
                    }
                    termLocal[argSortA[a]] += value;
                    termLocal[argSortB[a] + mesh.dVertex] += valuePrime;
                    if (conf.is_singularKernel){
                        for (int b = 0; b < nEqual; ++b) {
                            termNonloc[argSortA[a]*2*num_vertices + argSortA[b]]+= -c * value
                                                                                   * (x_canceled[0] - y_canceled[0])/(vec_sqL2dist(x_canceled, y_canceled, 2))
                            ...
                            termNonloc[(argSortB[a] + mesh.dVertex)*2*num_vertices + argSortA[b]]+= -c * valuePrime
                                                                                                    * (x_canceled[0] - y_canceled[0])/(vec_sqL2dist(x_canceled, y_canceled, 2))
                                                                                                    * psixy[b];
                            termNonloc[(argSortB[a] + mesh.dVertex)*2*num_vertices + argSortA[b] + num_vertices]+= -c * valuePrime
                                                                                                                   * (x_canceled[1] - y_canceled[1])/(vec_sqL2dist(x_canceled, y_canceled, 2))
                                                                                                                   * psixy[b];
                        }
                        for (int b = nEqual; b < mesh.dVertex; ++b) {
                            ...
                            termNonloc[(argSortB[a] + mesh.dVertex)*2*num_vertices + argSortB[b] + mesh.dVertex]+= -c * valuePrime
                                                                                               * (x_canceled[0] - y_canceled[0])/(vec_sqL2dist(x_canceled, y_canceled, 2))
                                                                                               * psixy[b + shift];
                            termNonloc[(argSortB[a] + mesh.dVertex)*2*num_vertices + argSortB[b] + mesh.dVertex + num_vertices]+= -(mesh.outdim + 2*mesh.fractional_s)* valuePrime
                                                                                                              * (x_canceled[1] - y_canceled[1])/(vec_sqL2dist(x_canceled, y_canceled, 2))
                                                                                                              * psixy[b + shift];
        ...
        if (Rdx == 0) {
            ...
            for (rTdx = 0; rTdx < Rdx; rTdx++) {
                for (i = 0; i < quadRule.nPy; i++) {
                    ...
                    if (conf.is_singularKernel){
                        for(a = 0; a < mesh.dVertex; a++) {
                            termNonloc[a] += -(mesh.outdim + 2*mesh.fractional_s)*value * (x[0] - physical_quad[0])/(vec_sqL2dist(x, physical_quad, 2))
                                    * quadRule.psix(a, k);
                            termNonlocPrime[a] += -(mesh.outdim + 2*mesh.fractional_s)*value *
...
int integrate_retriangulate_second_shape(const ElementType &aT, const ElementType &bT, const QuadratureType &quadRule,
                                         const MeshType &mesh, const ConfigurationType &conf, bool is_firstbfslayer,
                                         double *termLocal, double *termNonloc, double *termLocalPrime,
                                         double *termNonlocPrime, double * state_nodal_values,
                                         double * adjoint_nodal_values, const int CaseShapeDerivative) {
    // This procedure will maybe be included in integrate_retriangulate_shape
    const int dim = mesh.dim;
    int k = 0, a = 0;
    double x[dim];
    double state_values[2] = {0.0, 0.0};
                ...
                for (i = 0; i < quadRule.nPy; i++) {
                    ...
                    if(CaseShapeDerivative == 1) {
                        ...
                        if(conf.is_singularKernel){
                            ...
                            if (conf.ShapeDerivative==1){
                                for(a = 0; a < mesh.dVertex; a++) {
                                    termNonloc[a] += -c*value * (x[0] - physical_quad[0])/(vec_sqL2dist(x, physical_quad, 2))
                                                    * quadRule.psix(a, k);
                                    termNonloc[a + num_vertices] += -c*value *
                                                          (x[1] - physical_quad[1])/(vec_sqL2dist(x, physical_quad, 2)) * quadRule.psix(a, k);
                                    termNonloc[a + mesh.dVertex] += c*value * (x[0] - physical_quad[0])/(vec_sqL2dist(x, physical_quad, 2))
                                                                    * psi_value[a];
                                    termNonloc[a + mesh.dVertex + num_vertices] += c*value *
                                                                    (x[1] - physical_quad[1])/(vec_sqL2dist(x, physical_quad, 2)) * psi_value[a];
                                }
                            } else {
                                value = (adjoint_values[0] - adjoint_values[1]) * (state_values[0] - state_values[1])
                                        * quadRule.dy[i] * quadRule.dx[k] * rTdet * aT.absDet;

                                value11 = value * kernel_val[0] * (-c * 1 / (vec_sqL2dist(x, physical_quad, 2)) +
                                                                   (mesh.outdim * c + pow(c, 2))
                                                                   * (pow(x[0] - physical_quad[0], 2)) /
                                                                   pow(vec_sqL2dist(x, physical_quad, 2),2));
                                value22 = value * kernel_val[0] * (-c * 1 / (vec_sqL2dist(x, physical_quad, 2)) +
                                                                   (mesh.outdim * c + pow(c, 2))
                                                                   * (pow(x[1] - physical_quad[1], 2)) /
                                                                   pow(vec_sqL2dist(x, physical_quad, 2),2));
                                value12 = value * kernel_val[0] * (-c * 1 / (vec_sqL2dist(x, physical_quad, 2)) +
                                                                   (mesh.outdim * c + pow(c, 2))
                                                                   * ((x[0] - physical_quad[0]) *
                                                                      (x[1] - physical_quad[1])) /
                                                                   pow(vec_sqL2dist(x, physical_quad, 2),2));
                                for (int a = 0; a < mesh.dVertex; a++) {
                                    for (int b = 0; b < mesh.dVertex; b++) {
                                        // V^T Hess W
                                        //both basis functions with supp on first triangle
                                        termLocalPrime[a * 2 * num_vertices + b] += value11 * quadRule.psix(a, k) *
                                                                                    quadRule.psix(b, k);
                                        termLocalPrime[a * 2 * num_vertices + b + num_vertices] +=
                                                value12 * quadRule.psix(a, k)
                                                * quadRule.psix(b, k);
                                        termLocalPrime[(a + num_vertices) * 2 * num_vertices + b] +=
                                                value12 * quadRule.psix(a, k)
                                                * quadRule.psix(b, k);
                                        termLocalPrime[(a + num_vertices) * 2 * num_vertices + b + num_vertices] +=
                                                value22 * quadRule.psix(a, k)
                                                * quadRule.psix(b, k);

                                        //first function supp on first triangle, second function supp on second triangle
                                        termLocalPrime[a * 2 * num_vertices + b + mesh.dVertex] +=
                                                -value11 * quadRule.psix(a, k)
                                                * psi_value[b];
                                        termLocalPrime[a * 2 * num_vertices + b + num_vertices + mesh.dVertex] +=
                                                -value12 * quadRule.psix(a, k)
                                                * psi_value[b];
                                        termLocalPrime[(a + num_vertices) * 2 * num_vertices + b + mesh.dVertex] +=
                                                -value12 * quadRule.psix(a, k)
                                                * psi_value[b];
                                        termLocalPrime[(a + num_vertices) * 2 * num_vertices + b + num_vertices +
                                                       mesh.dVertex] += -value22 * quadRule.psix(a, k)
                                                                        * psi_value[b];

                                        //both basis functions with supp on second triangle
                                        termLocalPrime[(a + mesh.dVertex) * 2 * num_vertices + b + mesh.dVertex] +=
                                                value11 * psi_value[a]
                                                * psi_value[b];
                                        termLocalPrime[(a + mesh.dVertex) * 2 * num_vertices + b + mesh.dVertex +
                                                       num_vertices] += value12 * psi_value[a]
                                                                        * psi_value[b];
                                        termLocalPrime[(a + mesh.dVertex + num_vertices) * 2 * num_vertices + b +
                                                       mesh.dVertex] += value12 * psi_value[a]
                                                                        * psi_value[b];
                                        termLocalPrime[(a + mesh.dVertex + num_vertices) * 2 * num_vertices + b +
                                                       mesh.dVertex + num_vertices] += value22 * psi_value[a]
                                                                                       * psi_value[b];

                                        //first function supp on second triangle, second function supp on first triangle
                                        termLocalPrime[(a + mesh.dVertex) * 2 * num_vertices + b] +=
                                                -value11 * psi_value[a]
                                                * quadRule.psix(b, k);
                                        termLocalPrime[(a + mesh.dVertex) * 2 * num_vertices + b + num_vertices] +=
                                                -value12 * psi_value[a]
                                                * quadRule.psix(b, k);
                                        termLocalPrime[(a + mesh.dVertex + num_vertices) * 2 * num_vertices + b] +=
                                                -value12 * psi_value[a]
                                                * quadRule.psix(b, k);
                                        termLocalPrime[(a + mesh.dVertex + num_vertices) * 2 * num_vertices + b +
                                                       num_vertices] += -value22 * psi_value[a]
                                                                        * quadRule.psix(b, k);
                                    }
                                    // grad_x kernel^T(V-V') or grad_x kernel^T(W-W')
                                    termNonlocPrime[a] += -c * value * kernel_val[0] * quadRule.psix(a, k)
                                                          * (x[0] - physical_quad[0]) /
                                                          (vec_sqL2dist(x, physical_quad, 2));
                                    termNonlocPrime[a + mesh.dVertex] += c * value * kernel_val[0] * psi_value[a]
                                                                         * (x[0] - physical_quad[0]) /
                                                                         (vec_sqL2dist(x, physical_quad, 2));
                                    termNonlocPrime[a + num_vertices] +=
                                            -c * value * kernel_val[0] * quadRule.psix(a, k)
                                            * (x[1] - physical_quad[1]) / (vec_sqL2dist(x, physical_quad, 2));
                                    termNonlocPrime[a + num_vertices + mesh.dVertex] +=
                                            c * value * kernel_val[0] * psi_value[a]
                                            * (x[1] - physical_quad[1]) / (vec_sqL2dist(x, physical_quad, 2));
                                }
                            }
                        }
                        ...
                        for (int a = 0; a < mesh.dVertex; a++) {
                            if (CaseShapeDerivative == 2) {
                                value = (state_values[0]*kernel_val[0] - state_values[1]*kernelPrime_val[0])
                                        * quadRule.psix(a, k) * quadRule.dy[i]*quadRule.dx[k]*rTdet*aT.absDet;
                                valuePrime = -(state_values[0]*kernel_val[0] - state_values[1]*kernelPrime_val[0])
                                             * psi_value[a] * quadRule.dy[i]*quadRule.dx[k]*rTdet*aT.absDet;
                            } else {
                                value = (adjoint_values[0] - adjoint_values[1])
                                        *quadRule.psix(a, k)*kernel_val[0]
                                        *quadRule.dy[i]*quadRule.dx[k]*rTdet*aT.absDet;
                                valuePrime = -(adjoint_values[0] - adjoint_values[1])
                                             *psi_value[a]*kernelPrime_val[0]
                                             *quadRule.dy[i]*quadRule.dx[k]*rTdet*aT.absDet;
                            }
                            termLocal[a] += value;
                            termLocal[a + mesh.dVertex] += valuePrime;
                            if (conf.is_singularKernel) {
                                int num_vertices = 2*mesh.dVertex;
                                for (int b = 0; b < mesh.dVertex; ++b) {
                                    termNonloc[a*2*num_vertices + b]+= -c * value
                                            * (x[0] - physical_quad[0])/(vec_sqL2dist(x, physical_quad, 2))
                                             * quadRule.psix(b, k);
                                    termNonloc[a*2*num_vertices + b + num_vertices]+= -c * value
                                                                     * (x[1] - physical_quad[1])/(vec_sqL2dist(x, physical_quad, 2))
                                                                     * quadRule.psix(b, k);

                                    termNonloc[a*2*num_vertices + b + mesh.dVertex]+= c * value
                                                                     * (x[0] - physical_quad[0])/(vec_sqL2dist(x, physical_quad, 2))
                                                                     * psi_value[b];
                                    termNonloc[a*2*num_vertices + b + mesh.dVertex + num_vertices]+= c * value
                                                                                    * (x[1] - physical_quad[1])/(vec_sqL2dist(x, physical_quad, 2))
                                                                                    * psi_value[b];

                                    termNonloc[(a + mesh.dVertex)*2*num_vertices + b + mesh.dVertex]+= c * valuePrime
                                                                                      * (x[0] - physical_quad[0])/(vec_sqL2dist(x, physical_quad, 2))
                                                                                      * psi_value[b];
                                    termNonloc[(a + mesh.dVertex)*2*num_vertices + b + mesh.dVertex + num_vertices]+= c * valuePrime
                                                                                                     * (x[1] - physical_quad[1])/(vec_sqL2dist(x, physical_quad, 2))
                                                                                                     * psi_value[b];

                                    termNonloc[(a + mesh.dVertex)*2*num_vertices + b]+= -c * valuePrime
                                                                                                       * (x[0] - physical_quad[0])/(vec_sqL2dist(x, physical_quad, 2))
                                                                                                       * quadRule.psix(b, k);
                                    termNonloc[(a + mesh.dVertex)*2*num_vertices + b + num_vertices]+= -c * valuePrime
                                                                                                     * (x[1] - physical_quad[1])/(vec_sqL2dist(x, physical_quad, 2))
                                                                                                     * quadRule.psix(b, k);
                                }
                            }
