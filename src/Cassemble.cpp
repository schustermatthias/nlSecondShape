...
void lookup_configuration(ConfigurationType & conf, int verbose=0){
    ...
    map<string, void (*)(const double * x, long labelx, const double * y, long labely,
            ...
            {"constant_neumann", kernel_constant_neumann},
            {"schwarz_neumann", kernel_schwarz_neumann},
            ...
            {"integrable_unsym_1", kernel_integrable_unsym_1},
            {"integrable_unsym_2", kernel_integrable_unsym_2},
            ...
            {"singular", kernel_singular},
            {"DEliaGlusa", kernel_DEliaGlusa},
    ...
    if (conf.ShapeDerivative) {
        map<string, int (*)(const ElementType &aT, const ElementType &bT, const QuadratureType &quadRule, const MeshType &mesh,
                            ...
                            const int CaseShapeDerivative)> lookup_integrate {
                {"retriangulate_shape", integrate_retriangulate_shape},
                {"retriangulate_second_shape", integrate_retriangulate_second_shape},
                {"fractional_shape", integrate_fractional_shape},
                {"fractional_second_shape", integrate_fractional_second_shape},
                {"ERROR_wrongAccess_shape", ERROR_wrongAccess_shape}
        };
        map<string, int (*)(const ElementType &aT, const ElementType &bT, const QuadratureType &quadRule, const MeshType &mesh,
                            const ConfigurationType &conf, bool is_firstbfslayer, double *termLocal, double *termNonloc,
                            double *termLocalPrime, double *termNonlocPrime, double *state_nodal_values, double *adjoint_nodal_values,
                            const int CaseShapeDerivative)>::iterator it_integrator;
...
void par_assemble(const string compute, const string path_spAd, const string path_fd, const int K_Omega, const int K,
                  ...
                  int ShapeDerivative, int dg_solutions, const double *state, const double *adjoint
                  ) {
...
void par_system(MeshType &mesh, QuadratureType &quadRule, ConfigurationType &conf,
    ...
    int CaseShapeDerivative = 0;
    if (conf.ShapeDerivative >= 1) {
        if ((state != nullptr) && (adjoint != nullptr)) {
            CaseShapeDerivative = 1;
        } else if (state != nullptr) {
            CaseShapeDerivative = 2;
        } else if (adjoint != nullptr){
            CaseShapeDerivative = 3;
        } else {
            printf("State and/or adjoint missing for shape derivative.");
        }
    }
    ...
    #pragma omp parallel default(none) shared(cout, mesh, quadRule, conf, values_all, indices_all, nnz_total, epart, nparts, verbose, zeros, state, adjoint, CaseShapeDerivative, num_vertices)
    {
    map<unsigned long, double> Ad;
    unsigned long Adx;
    idx_t tid = 0;
    ...
    const long * aAdx;
    const long * bAdx;
    const long * indices_solution;
    long aDGdx[mesh.dVertex]; // Index for discontinuous Galerkin
    long bDGdx[mesh.dVertex];

    // Buffers for integration solutions
    double termLocal[4*mesh.K];
    double termNonloc[4*mesh.K];
    double termLocalPrime[4*mesh.K];
    double termNonlocPrime[4*mesh.K];
    //#pragma omp for
    for (int aTdx=0; aTdx<mesh.nE; aTdx++) {
                    ...
                    //if (bTdx < mesh.nE) {
                        ...
                        if (!visited[bTdx]) {
                            ...
                            if (mesh.LabelTriangles[bTdx]) {
                                ...
                                // Assembly of matrix ---------------------------------------
                                doubleVec_tozero(termLocal, 4*mesh.K); // Initialize Buffer
                                doubleVec_tozero(termNonloc, 4*mesh.K); // Initialize Buffer
                                doubleVec_tozero(termLocalPrime, 4*mesh.K); // Initialize Buffer
                                doubleVec_tozero(termNonlocPrime, 4*mesh.K); // Initialize Buffer

                                // Compute integrals and write to buffer
                                int doesInteract = 0;
                                if (conf.ShapeDerivative) {
                                    if (conf.dg_solutions) {
                                        for (int j = 0; j < mesh.dVertex; j++) {
                                            bDGdx[j] = mesh.dVertex * bTdx + j;
                                        }
                                        indices_solution = bDGdx;
                                    } else {
                                        indices_solution = &mesh.Triangles(0, bTdx);
                                    }
                                }
                                if (conf.ShapeDerivative >= 1) {
                                    if (CaseShapeDerivative == 1) {
                                        for (int i=0; i < mesh.dVertex; i++) {
                                            state_nodal_values[i + mesh.dVertex] = state[indices_solution[i]];
                                            adjoint_nodal_values[i + mesh.dVertex] = adjoint[indices_solution[i]];
                                        }
                                    } else if (CaseShapeDerivative == 2) {
                                        for (int i = 0; i < mesh.dVertex; i++) {
                                            state_nodal_values[i + mesh.dVertex] = state[indices_solution[i]];
                                    ...
                                    doesInteract = integrate_shape(aT, bT, quadRule, mesh, conf, is_firstbfslayer,
                                                                   termLocal, termNonloc, termLocalPrime,
                                                                   termNonlocPrime, &state_nodal_values[0],
                                                                   &adjoint_nodal_values[0], CaseShapeDerivative);
                                } else {
                                        ...
                                        if (CaseShapeDerivative == 1){
                                            for (int a = 0; a < mesh.dVertex; a++) {
                                                if (mesh.LabelVerts[aAdx[a]] > 0) {
                                                    {
                                                        Ad[aAdx[a]] += 2 * termLocal[0] * div[2 * a] + termNonloc[a];
                                                        Ad[aAdx[a] + mesh.K] += 2 * termLocal[0] * div[2 * a + 1]
                                                                                + termNonloc[a + num_vertices];
                                                    }
                                                }
                                                if (mesh.LabelVerts[bAdx[a]] > 0){
                                                    // This could also include a div term, if I delete the factor 2 above and if I use div_a, div_b
                                                    Ad[bAdx[a]] += termNonloc[a + mesh.dVertex];
                                                    Ad[bAdx[a] + mesh.K] += termNonloc[a + mesh.dVertex + num_vertices];
                                                }
                                            }
                                            ...
                                            for (int a = 0; a < mesh.dVertex; a++) {
                                                if (mesh.LabelVerts[aAdx[a]] > 0) {
                                                    {
                                                        cout << endl << "This case is deprecated at the moment and might not work properly." << endl ;
                                                        Ad[aAdx[a]] += termLocal[a];
                                                        Ad[bAdx[a]] += -termLocal[a + mesh.dVertex];
                                                    }
                                                }
                                            }
                                        }

                                    } else if (conf.ShapeDerivative == 2) {
                                        double  div_a[6];
                                        get_div(aT.E, &div_a[0]);
                                        double  div_b[6];
                                        get_div(bT.E, &div_b[0]);
                                       if (CaseShapeDerivative == 1) {
                                           double traces[9][4];
                                           get_traces(&div_a[0], &div_a[0], traces);

                                           for (int a = 0; a < mesh.dVertex; a++) {
                                               for (int b = 0; b < mesh.dVertex; b++) {
                                                   if (mesh.LabelVerts[aAdx[a]] > 0) {
                                                       {
                                                           // Ad[aAdx[a] * 2 * mesh.K + bAdx[b]]
                                                           // += termLocal[0] * (div_a[2*a] * div_b[2*b]
                                                           // - 2 * traces[3*a + b][0]);
                                                           //Product of divergences
                                                           Ad[aAdx[a] * 2 * mesh.K + aAdx[b]]
                                                           += termLocal[0] * div_a[2*a] * div_a[2*b];
                                                           Ad[aAdx[a] * 2 * mesh.K + aAdx[b] + mesh.K]
                                                           += termLocal[0] * div_a[2*a] * div_a[2*b + 1];
                                                           Ad[mesh.K * 2 * mesh.K + aAdx[a] * 2 * mesh.K + aAdx[b]]
                                                           += termLocal[0] * div_a[2*a + 1] * div_a[2*b];
                                                           Ad[mesh.K * 2 * mesh.K + aAdx[a] * 2 * mesh.K + aAdx[b] + mesh.K]
                                                           += termLocal[0] * div_a[2*a + 1] * div_a[2*b + 1];

                                                           Ad[aAdx[a] * 2 * mesh.K + bAdx[b]]
                                                                   += termLocal[0] * div_a[2*a] * div_b[2*b];
                                                           Ad[aAdx[a] * 2 * mesh.K + bAdx[b] + mesh.K]
                                                                   += termLocal[0] * div_a[2*a] * div_b[2*b + 1];
                                                           Ad[mesh.K * 2 * mesh.K + aAdx[a] * 2 * mesh.K + bAdx[b]]
                                                                   += termLocal[0] * div_a[2*a + 1] * div_b[2*b];
                                                           Ad[mesh.K * 2 * mesh.K + aAdx[a] * 2 * mesh.K + bAdx[b] + mesh.K]
                                                                   += termLocal[0] * div_a[2*a + 1] * div_b[2*b + 1];

                                                           Ad[bAdx[a] * 2 * mesh.K + bAdx[b]]
                                                                   += termLocal[0] * div_b[2*a] * div_b[2*b];
                                                           Ad[bAdx[a] * 2 * mesh.K + bAdx[b] + mesh.K]
                                                                   += termLocal[0] * div_b[2*a] * div_b[2*b + 1];
                                                           Ad[mesh.K * 2 * mesh.K + bAdx[a] * 2 * mesh.K + bAdx[b]]
                                                                   += termLocal[0] * div_b[2*a + 1] * div_b[2*b];
                                                           Ad[mesh.K * 2 * mesh.K + bAdx[a] * 2 * mesh.K + bAdx[b] + mesh.K]
                                                                   += termLocal[0] * div_b[2*a + 1] * div_b[2*b + 1];

                                                           Ad[bAdx[a] * 2 * mesh.K + aAdx[b]]
                                                                   += termLocal[0] * div_b[2*a] * div_a[2*b];
                                                           Ad[bAdx[a] * 2 * mesh.K + aAdx[b] + mesh.K]
                                                                   += termLocal[0] * div_b[2*a] * div_a[2*b + 1];
                                                           Ad[mesh.K * 2 * mesh.K + bAdx[a] * 2 * mesh.K + aAdx[b]]
                                                                   += termLocal[0] * div_b[2*a + 1] * div_a[2*b];
                                                           Ad[mesh.K * 2 * mesh.K + bAdx[a] * 2 * mesh.K + aAdx[b] + mesh.K]
                                                                   += termLocal[0] * div_b[2*a + 1] * div_a[2*b + 1];

                                                           //traces
                                                           //todo maybe make this also symmetric
                                                           Ad[aAdx[a] * 2 * mesh.K + aAdx[b]]
                                                           += -2 * termLocal[0] * traces[3*a + b][0];
                                                           Ad[aAdx[a] * 2 * mesh.K + aAdx[b] + mesh.K]
                                                                   += -2 * termLocal[0] * traces[3*a + b][1];
                                                           Ad[mesh.K * 2 * mesh.K + aAdx[a] * 2 * mesh.K + aAdx[b]]
                                                                   += -2 * termLocal[0] * traces[3*a + b][2];
                                                           Ad[mesh.K * 2 * mesh.K + aAdx[a] * 2 * mesh.K + aAdx[b] + mesh.K]
                                                                   += -2 * termLocal[0] * traces[3*a + b][3];
                                                           if (conf.is_singularKernel){
                                                               //...(V^T Hess(kernel)W + (grad kernel^T V)*div W + (grad kernel^T W)*div V)
                                                               Ad[aAdx[a] * 2 * mesh.K + aAdx[b]]
                                                                       += termLocalPrime[a*2*num_vertices + b] + termNonlocPrime[a]*2*div_a[2*b] + termNonlocPrime[b]*2*div_a[2*a];
                                                               Ad[aAdx[a] * 2 * mesh.K + aAdx[b] + mesh.K]
                                                                       += termLocalPrime[a*2*num_vertices + b + num_vertices] + termNonlocPrime[a]*2*div_a[2*b+1]+ termNonlocPrime[b + num_vertices]*2*div_a[2*a];
                                                               Ad[mesh.K * 2 * mesh.K + aAdx[a] * 2 * mesh.K + aAdx[b]]
                                                                       += termLocalPrime[(a + num_vertices)*2*num_vertices + b] + termNonlocPrime[a + num_vertices]*2*div_a[2*b] + termNonlocPrime[b]*2*div_a[2*a + 1];
                                                               Ad[mesh.K * 2 * mesh.K + aAdx[a] * 2 * mesh.K + aAdx[b] + mesh.K]
                                                                       += termLocalPrime[(a + num_vertices)*2*num_vertices + b + num_vertices] + termNonlocPrime[a + num_vertices]*2*div_a[2*b + 1] + termNonlocPrime[b + num_vertices]*2*div_a[2*a + 1];

                                                               Ad[aAdx[a] * 2 * mesh.K + bAdx[b]]
                                                                       += termLocalPrime[a*2*num_vertices + b + mesh.dVertex] + termNonlocPrime[a]*2*div_b[2*b] + termNonlocPrime[b + mesh.dVertex]*2*div_a[2*a];
                                                               Ad[aAdx[a] * 2 * mesh.K + bAdx[b] + mesh.K]
                                                                       += termLocalPrime[a*2*num_vertices + b + num_vertices + mesh.dVertex] + termNonlocPrime[a]*2*div_b[2*b + 1] + termNonlocPrime[b + mesh.dVertex + num_vertices]*2*div_a[2*a];
                                                               Ad[mesh.K * 2 * mesh.K + aAdx[a] * 2 * mesh.K + bAdx[b]]
                                                                       += termLocalPrime[(a + num_vertices)*2*num_vertices + b + mesh.dVertex] + termNonlocPrime[a + num_vertices]*2*div_b[2*b] + termNonlocPrime[b + mesh.dVertex]*2*div_a[2*a + 1];
                                                               Ad[mesh.K * 2 * mesh.K + aAdx[a] * 2 * mesh.K + bAdx[b] + mesh.K]
                                                                       += termLocalPrime[(a + num_vertices)*2*num_vertices + b + num_vertices + mesh.dVertex] + termNonlocPrime[a + num_vertices]*2*div_b[2*b + 1] + termNonlocPrime[b + mesh.dVertex + num_vertices]*2*div_a[2*a + 1];

                                                               Ad[bAdx[a] * 2 * mesh.K + bAdx[b]]
                                                                       += termLocalPrime[(a + mesh.dVertex)*2*num_vertices + b + mesh.dVertex] + termNonlocPrime[a + mesh.dVertex]*2*div_b[2*b] + termNonlocPrime[b + mesh.dVertex]*2*div_b[2*a];
                                                               Ad[bAdx[a] * 2 * mesh.K + bAdx[b] + mesh.K]
                                                                       += termLocalPrime[(a + mesh.dVertex)*2*num_vertices + b + mesh.dVertex + num_vertices] + termNonlocPrime[a + mesh.dVertex]*2*div_b[2*b + 1] + termNonlocPrime[b + mesh.dVertex + num_vertices]*2*div_b[2*a];
                                                               Ad[mesh.K * 2 * mesh.K + bAdx[a] * 2 * mesh.K + bAdx[b]]
                                                                       += termLocalPrime[(a + mesh.dVertex + num_vertices)*2*num_vertices + b + mesh.dVertex] + termNonlocPrime[a + mesh.dVertex + num_vertices]*2*div_b[2*b] + termNonlocPrime[b + mesh.dVertex]*2*div_b[2*a + 1];
                                                               Ad[mesh.K * 2 * mesh.K + bAdx[a] * 2 * mesh.K + bAdx[b] + mesh.K]
                                                                       += termLocalPrime[(a + mesh.dVertex + num_vertices)*2*num_vertices + b + mesh.dVertex + num_vertices] + termNonlocPrime[a + mesh.dVertex + num_vertices]*2*div_b[2*b + 1] + termNonlocPrime[b + mesh.dVertex + num_vertices]*2*div_b[2*a + 1];

                                                               Ad[bAdx[a] * 2 * mesh.K + aAdx[b]]
                                                                       += termLocalPrime[(a + mesh.dVertex)*2*num_vertices + b] + termNonlocPrime[a + mesh.dVertex]*2*div_a[2*b] + termNonlocPrime[b + mesh.dVertex]*2*div_a[2*a];
                                                               Ad[bAdx[a] * 2 * mesh.K + aAdx[b] + mesh.K]
                                                                       += termLocalPrime[(a + mesh.dVertex)*2*num_vertices + b + num_vertices] + termNonlocPrime[a + mesh.dVertex]*2*div_a[2*b + 1] + termNonlocPrime[b + mesh.dVertex + num_vertices]*2*div_a[2*a];
                                                               Ad[mesh.K * 2 * mesh.K + bAdx[a] * 2 * mesh.K + aAdx[b]]
                                                                       += termLocalPrime[(a + mesh.dVertex + num_vertices)*2*num_vertices + b] + termNonlocPrime[a + mesh.dVertex + num_vertices]*2*div_a[2*b] + termNonlocPrime[b + mesh.dVertex]*2*div_a[2*a + 1];
                                                               Ad[mesh.K * 2 * mesh.K + bAdx[a] * 2 * mesh.K + aAdx[b] + mesh.K]
                                                                       += termLocalPrime[(a + mesh.dVertex + num_vertices)*2*num_vertices + b + num_vertices] + termNonlocPrime[a + mesh.dVertex + num_vertices]*2*div_a[2*b + 1] + termNonlocPrime[b + mesh.dVertex + num_vertices]*2*div_a[2*a + 1];
                                                           }
                                                       }
                                                   }
                                               }
                                           ...
                                           for (int a = 0; a < mesh.dVertex; a++) {
                                               for (int b = 0; b < mesh.dVertex; b++) {
                                                   if (mesh.LabelVerts[aAdx[a]] > 0) {
                                                       {
                                                           Ad[aAdx[a] * 2 * mesh.K + aAdx[b]]
                                                           += termLocal[a] * div_a[2*b] + termNonloc[a*2*num_vertices + b];
                                                           Ad[aAdx[a] * 2 * mesh.K + aAdx[b] + mesh.K]
                                                           += termLocal[a] * div_a[2*b + 1] + termNonloc[a*2*num_vertices + b + num_vertices];

                                                           Ad[aAdx[a] * 2 * mesh.K + bAdx[b]]
                                                                   += termLocal[a] * div_b[2*b] + termNonloc[a*2*num_vertices + b + mesh.dVertex];
                                                           Ad[aAdx[a] * 2 * mesh.K + bAdx[b] + mesh.K]
                                                                   += termLocal[a] * div_b[2*b + 1] + termNonloc[a*2*num_vertices + b + mesh.dVertex + num_vertices];
                                                       }
                                                   }
                                                   if (mesh.LabelVerts[bAdx[a]] > 0) {
                                                       Ad[bAdx[a] * 2 * mesh.K + bAdx[b]]
                                                               += termLocal[a + mesh.dVertex] * div_b[2*b] + termNonloc[(a + mesh.dVertex)*2*num_vertices + b + mesh.dVertex];
                                                       Ad[bAdx[a] * 2 * mesh.K + bAdx[b] + mesh.K]
                                                               += termLocal[a + mesh.dVertex] * div_b[2*b + 1] + termNonloc[(a + mesh.dVertex)*2*num_vertices + b + mesh.dVertex + num_vertices];

                                                       Ad[bAdx[a] * 2 * mesh.K + aAdx[b]]
                                                               += termLocal[a + mesh.dVertex] * div_a[2*b] + termNonloc[(a + mesh.dVertex)*2*num_vertices + b];
                                                       Ad[bAdx[a] * 2 * mesh.K + aAdx[b] + mesh.K]
                                                               += termLocal[a + mesh.dVertex] * div_a[2*b + 1] + termNonloc[(a + mesh.dVertex)*2*num_vertices + b + num_vertices];
                                                   }
                                               }
                                           }
                                       }
...
//#pragma omp critical
    ...
    */
    ...
    #pragma omp critical
        {
            ...
            for (auto & it : Ad) {
                ...
                if (conf.ShapeDerivative == 1) {
                    ...
                    //every row has 2*(number of nodes) entries(due to div V) and there are 2*(number of nodes) columns
                    indices_all(0, nnz_start + k) = adx % (2 * mesh.K);
                    indices_all(1, nnz_start + k) = adx / (2 * mesh.K);
                } else {
                    // column major format of transposed matrix Ad
    ...
    //location of values_all(i) is: row indices_all(0,i)    column indices_all(1,i)
    sp_Ad.save(conf.path_spAd);
