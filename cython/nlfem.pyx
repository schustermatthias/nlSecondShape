...
def stiffnessMatrix(
    ...
    cdef int ShapeDerivative = configuration.get("ShapeDerivative", 0)
    if (verbose and not ShapeDerivative): print("stiffnessMatrix(): ShapeDerivative not found or 0 (default).")

    cdef int dg_solutions = configuration.get("dg_solutions", 0)
    if (verbose and not ShapeDerivative): print("stiffnessMatrix(): dg_solutions not found or 0 (default).")
    ...
    if ShapeDerivative:
        state = shape_derivative_data.get('state', [])
        adjoint = shape_derivative_data.get('adjoint', [])
    ...
    Cassemble.par_assemble( "system".encode('UTF-8'), path_spAd_,
                            ...
                            fractional_s,
                             ...
                             verbose,
                             ShapeDerivative, dg_solutions, &state[0], &adjoint[0])
...
def loadVector(
    ...
    Cassemble.par_assemble( "forcing".encode('UTF-8'), "".encode('UTF-8'),
                            ...
                            ptrZetaIndicator_indptr,
                            nZeta,
                            NULL, 0, NULL, 0.0, -1.0, 0, verbose, 0, 0, NULL, NULL)
...
def assemble(
    ...
    cdef int ShapeDerivative = 0
    cdef int dg_solutions = 0
    try:
    ...
    if (compute=="system" or compute=="systemforcing"):
        ...
        Cassemble.par_assemble( compute_system_, path_spAd_, path_fd_, mesh.K_Omega, mesh.K,
                            ...
                            verbose, ShapeDerivative, dg_solutions, NULL, NULL)
    ...
    if (compute=="forcing" or compute =="systemforcing"):
        ...
        Cassemble.par_assemble( compute_forcing_, path_spAd_, path_fd_, mesh.K_Omega, mesh.K,
                            ...
                            mesh.dim, outdim,  NULL, NULL, 0,
                            ptrPg, tensorGaussDegree, ptrdg, maxDiameter, -1.0, is_fullConnectedComponentSearch,
                            verbose, ShapeDerivative, 0, NULL, NULL)
